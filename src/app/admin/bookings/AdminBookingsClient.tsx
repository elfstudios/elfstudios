...
export function AdminBookingsClient({ initialBookings }: { initialBookings: Booking[] }) {
  ...
  async function submit() {
    ...
    const response = await fetch(`/api/admin/bookings/${active.booking.id}`, {
      ...
      body: JSON.stringify({ action: active.action, reason, requestedDate: date, requestedSlots: slots, bookingName, bandName, attendees, equipmentRequests }),
    });
  ...
  async function remove(booking: Booking) {
    if (!confirm(`Permanently delete cancelled booking ${booking.ticketNumber || ""}?`)) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {visible.map((booking) => (
          <article key={booking.id} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-widest text-orange-600">{booking.ticketNumber || "No ticket"}</p><h2 className="mt-1 text-xl font-black">{booking.bandName || "Unknown artist"}</h2><p className="text-sm text-gray-500">Booked by: {booking.bookingName || booking.user.name || booking.user.email}</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-bold ${booking.status === "CONFIRMED" ? "bg-green-100 text-green-700" : booking.status === "CANCELLED" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>{booking.status}</span></div>
      ...
      {active && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center sm:p-4"><div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-white p-5 sm:rounded-3xl sm:p-7">
        <div className="flex justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-orange-600">{active.booking.ticketNumber}</p><h2 className="text-2xl font-black">{active.action === "CANCEL" ? "Cancel booking" : active.action === "EDIT" ? "Edit booking" : "Reschedule booking"}</h2></div><button onClick={() => setActive(null)} aria-label="Close" className="h-11 w-11 rounded-full bg-gray-100 text-2xl">×</button></div>
        {active.action === "EDIT" && <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-widest text-gray-500">Booking name<input value={bookingName} onChange={(e) => setBookingName(e.target.value)} className="mt-2 h-12 w-full rounded-xl border px-3 text-base font-normal text-black"/></label><label className="text-xs font-bold uppercase tracking-widest text-gray-500">Artist name<input value={bandName} onChange={(e) => setBandName(e.target.value)} className="mt-2 h-12 w-full rounded-xl border px-3 text-base font-normal text-black"/></label><label className="text-xs font-bold uppercase tracking-widest text-gray-500">Attendees<input type="number" min="1" max="10" value={attendees} onChange={(e) => setAttendees(Number(e.target.value))} className="mt-2 h-12 w-full rounded-xl border px-3 text-base font-normal text-black"/></label><label className="text-xs font-bold uppercase tracking-widest text-gray-500">Equipment<textarea value={equipmentRequests} onChange={(e) => setEquipmentRequests(e.target.value)} className="mt-2 min-h-12 w-full rounded-xl border p-3 text-base font-normal text-black"/></label></div>}
        {active.action !== "CANCEL" && <div className="mt-6 space-y-4"><label className="block text-xs font-bold uppercase tracking-widest text-gray-500">{active.action === "EDIT" ? "Date" : "New date"}<input type="date" value={date} min={new Date().toISOString().slice(0,10)} onChange={(event) => loadAvailability(event.target.value)} className="mt-2 h-12 w-full rounded-xl border px-3 text-base font-normal text-black"/></label><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{HOURS.map((slot) => {const unavailable=booked.includes(slot);const selected=slots.includes(slot);return <button type="button" key={slot} disabled={!date || unavailable} onClick={() => setSlots((current) => selected ? current.filter((item) => item !== slot) : current.length < (active.action === "EDIT" ? 12 : active.booking.slots.length) ? [...current,slot] : current)} className={`min-h-12 rounded-xl border text-xs ${unavailable ? "bg-red-50 text-red-300" : selected ? "bg-black text-white" : "bg-white"}`}>{slotLabel(slot)}{unavailable && <span className="block text-[9px]">Booked</span>}</button>})}</div><p className="text-xs text-gray-500">{active.action === "EDIT" ? "Adjust duration or time as needed." : `Select exactly ${active.booking.slots.length} hour(s).`}</p></div>}
        ...
        <div className="mt-6 flex gap-3"><button onClick={() => setActive(null)} className="min-h-12 flex-1 rounded-xl border">Back</button><button disabled={submitting || (active.action !== "CANCEL" && (!date || !slots.length || (active.action === "RESCHEDULE" && slots.length !== active.booking.slots.length))) || (active.action === "EDIT" && (!bookingName.trim() || !bandName.trim()))} onClick={submit} className={`min-h-12 flex-[2] rounded-xl font-bold text-white disabled:opacity-40 ${active.action === "CANCEL" ? "bg-red-600" : "bg-black"}`}>{submitting ? "Saving…" : active.action === "CANCEL" ? "Confirm cancellation" : active.action === "EDIT" ? "Save booking" : "Confirm reschedule"}</button></div>
      </div></div>}
    </div>
  );
}
